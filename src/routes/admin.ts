...
async function audit(adminId: number, action: string, resource: string, details: string, ip: string | null) {
  await query(
    `INSERT INTO audit_logs (admin_id, action, resource, details, ip_address) VALUES ($1,$2,$3,$4,$5)`,
    [adminId, action, resource, details, ip]
  );
}

// Auth guard applied per-route, never via router.use()
const adminAuth = [requireSession, requireSessionAdmin];

router.get('/users', ...adminAuth, async (_req, res, next) => {
  try {
    const { rows } = await query(`
      SELECT u.id, u.email, u.full_name, u.organization, u.user_type,
...
router.delete('/users/:userId', ...adminAuth, async (req, res, next) => {
  try {
...
router.get('/api-keys', ...adminAuth, async (_req, res, next) => {
  try {
    const { rows } = await query(`
      SELECT ak.id, ak.name, ak.key_prefix, ak.is_active, ak.created_at, ak.last_used,
...
router.post('/api-keys', ...adminAuth, async (req, res, next) => {
  try {
    const { name } = req.body;
    ...
    const keyHash = await hashApiKey(raw);
    await query(
      `INSERT INTO api_keys (user_id, name, key_hash, key_prefix, is_active) VALUES ($1,$2,$3,$4,true)`,
      [userId, name, keyHash, prefix]
    );
    await audit(userId, 'CREATE_API_KEY', `api_key:${name}`, '', req.ip ?? null);
    res.status(201).json({ data: { name, api_key: raw, prefix, message: 'Store this API key securely. It will not be shown again.' } });
  } catch (e) { next(e); }
});
...
router.post('/import', ...adminAuth, upload.single('file'), async (req, res, next) => {
  ...
  try {
    ...
    const records = parse(req.file.buffer, { columns: true, skip_empty_lines: true, trim: true }) as Record<string, string>[];
    ...
    for (const r of records) {
      ...
      if (!geoCode || !indCode || isNaN(year) || isNaN(value)) { skipped++; continue; }
      ...
      if (!geo.rowCount || !ind.rowCount) { skipped++; continue; }
      ...
      await client.query(
        ...
        [geo.rows[0].id, ind.rows[0].id, year, value, r.gender || 'all', r.age_group || 'all', r.source || null]
      );
      inserted++;
    }
  ...
  try {
    ...
    const { rows: usage } = await query(
      `SELECT ul.id, ul.user_id, u.email, ul.endpoint, ul.method, ul.status_code,
       ...
       ORDER BY ul.timestamp DESC LIMIT $1`, [limit]
    );
...
router.get('/settings', ...adminAuth, async (_req, res, next) => {
  try {
    const { rows } = await query(`SELECT config_key, config_value, description, updated_at FROM system_config ORDER BY config_key`);
    const settings: Record<string, any> = {};
    for (const row of rows) {
      try { settings[row.config_key] = JSON.parse(row.config_value); }
      catch { settings[row.config_key] = row.config_value; }
    }
...
router.post('/settings', ...adminAuth, async (req, res, next) => {
  try {
    ...
    for (const [key, value] of Object.entries(settings)) {
      await query(
        `INSERT INTO system_config (config_key, config_value, updated_at) VALUES ($1,$2,NOW())
         ON CONFLICT (config_key) DO UPDATE SET config_value = EXCLUDED.config_value, updated_at = NOW()`,
        [key, JSON.stringify(value)]
      );
    }
    await query('COMMIT');
    await audit(getUserId(req), 'UPDATE_SETTINGS', 'system_config', `Updated ${Object.keys(settings).length} settings`, req.ip ?? null);
    res.json({ data: { message: 'Settings saved successfully', updated: Object.keys(settings) } });
  } catch (e) {
...
router.get('/stats', ...adminAuth, async (_req, res, next) => {
  try {
    ...
    `);
    const { rows: byType } = await query(`SELECT user_type, COUNT(*) AS count FROM users GROUP BY user_type ORDER BY user_type`);
    res.json({ data: { ...rows[0], users_by_type: byType } });
  } catch (e) { next(e); }
});
